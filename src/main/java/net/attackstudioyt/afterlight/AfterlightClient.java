...
import net.attackstudioyt.afterlight.screen.RevivalBeaconScreen;
...
public class AfterlightClient implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        ...
        ClientPlayNetworking.registerGlobalReceiver(GhostStatePayload.ID, (payload, context) -> {
            ...
            if (client.player != null && client.player.getUuid().equals(payload.playerUuid())) {
                ...
                if (!wasGhost && payload.isGhost()) {
                    Toast.show("§7Afterlight", "You are now a ghost.");
        ...
        // /studiolib afterlight toggle — force-exit ghost mode
        StudioCommand.register("afterlight", "toggle", "Force-exit ghost mode (debug)", ctx -> {
        ...
        // /studiolib afterlight visible — show visibility status (actual toggle is server-side via /afterlight visible)
        StudioCommand.register("afterlight", "visible", "Show your current ghost visibility status", ctx -> {
            ...
            ctx.getSource().sendFeedback(Text.literal(
                ...
                "\n§8Use §f/afterlight visible§8 to toggle."
            ));
            return 1;
        });
        ...
        UseItemCallback.EVENT.register((player, world, hand) -> {
            if (world.isClient() && player.getStackInHand(hand).isOf(AfterlightMod.REVIVAL_BEACON)) {
                MinecraftClient client = MinecraftClient.getInstance();
